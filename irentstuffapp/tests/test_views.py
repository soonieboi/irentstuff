...
#         self.assertEqual(str(cm.exception), "Rental cannot be cancelled after it has started.")
...
#         # Check that the response redirects to the item detail page
#         self.assertRedirects(response, reverse('item_detail', args=(self.item.id,)))

    # # test cancel rental after rent start date
    # def test_cancel_rental(self):
        
    #     # Log in as the user
    #     self.client.login(username='testuser', password='testpassword')

    #     # Set the end date of the rental to be in future, while rental has started
    #     rental = self.rental
    ...
    #     with self.assertRaises(ValidationError) as cm:
    #         cancel_rental(self.client.request().wsgi_request, rental.item.id)
    #     self.assertEqual(str(cm.exception), "Rental cannot be cancelled after it has started")
